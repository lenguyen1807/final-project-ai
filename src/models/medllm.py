...
class MedBLIPModel_biomedlm(Blip2Base):
    ...
    def __init__(
        ...
        self.proj = nn.Linear(
            self.Qformer.config.hidden_size, self.lm_model.config.n_embd
        )
    ...
    def forward(self, samples):
        ...
        for b in range(bs):
            ...
            if "The diagnosis is" in doc:
                ...
                question.append(
                    "What will this subject be diagnosed with?"
                )  # hard coded
        ...
        sim_q2t = torch.matmul(
            image_feats.unsqueeze(1), qa_feat.unsqueeze(-1)
        ).squeeze()
        sim_i2t, index_i2t = sim_q2t.max(-1)
        sim_i2t = sim_i2t / self.temp

        sim_t2q = torch.matmul(
            qa_feat.unsqueeze(1).unsqueeze(1), image_feats.permute(0, 2, 1)
        ).squeeze()
        sim_t2i, _ = sim_t2q.max(-1)
        sim_t2i = sim_t2i / self.temp
        ...
        )  # bs output_txt_len(<=max_len) 0的位置填-100
        output_targets = output_tokens.input_ids.masked_fill(
            output_tokens.input_ids == self.tokenizer.pad_token_id, -100
        )

        empty_img_targets = (
            torch.ones(atts_img.size(), dtype=torch.long)
            .to(image.device)
            .fill_(-100)  # bs 32
        )

        targets = torch.cat(
            [input_targets, empty_img_targets, output_targets], dim=1
        )  # bs 32+txt_len (这里的32都是-100不计算loss)

        inputs_txt_embeds = self.lm_model.transformer.wte(
            input_tokens.input_ids
        )  # bs txt_len 2560
        outputs_txt_embeds = self.lm_model.transformer.wte(
            output_tokens.input_ids
        )  # bs txt_len 2560
        inputs_embeds = torch.cat(
            [inputs_txt_embeds, img_embeds, outputs_txt_embeds], dim=1
        )

        attention_mask = torch.cat(
            [input_tokens.attention_mask, atts_img, output_tokens.attention_mask], dim=1
        )  # bs 32+input_txt_len
    ...
    def generate(
        ...
        filler_input_ids = (
            ...
            .to(image.device)
            .fill_(self.lm_model.config.bos_token_id)
            .to(image.device)
        )
